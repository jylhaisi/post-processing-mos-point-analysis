...
FetchData_season_analysis_time <- function(station_id, atime, mos_season_data, obs_season_data,response) {
  # 
      ...
      if ( length(param_name <- filter(mos_vars, mos_vars$param_id == as.integer(param_list[i]) & mos_vars$level_value == level_list[j])[[1]]) == 0) { 
        if (length(param_name <- row.names(mos_D_vars)[which(suppressWarnings(mos_D_vars$derived_param_id) == param_list[i] & suppressWarnings(as.integer(mos_D_vars$MOS_previ_ecmos_narrow_v_level)) == level_list[j])]) == 0) {
          next
	        ...
	        df <- (mos_season_data %>%
                dplyr::filter( analysis_time == atime, param_id == param_list[i], level_value == level_list[j]) %>%
...
CleanDataCategory <- function(data, category) {
  ...
  if (category == 0) {
    data00 <- filter(data, forecast_period == 0)
    cleaned_data_00 <- CleanData(data00)
  }
  
  if (category == 1) {
    data_1_144 <- filter(data, forecast_period >= 3 || forecast_period <= 144)
    cleaned_data_1_144 <- CleanData(data1_144)
  }
  
  if (category == 2) {
    data_150_240 <- filter(data, forecast_period >= 150 || forecast_period <= 240)
    cleaned_data_150_240 <- CleanData(data_150_240)
  }
...
FitWithGlmnM1<- function(training.set) {
  # The Lasso regression which takes 11 predictors and lambda min
  #
  # Args:
  #   training.set:
  # glmnet takes the model data and observations as arguements
