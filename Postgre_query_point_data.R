...
for (station_number_index in station_numbers_indices) {
  ...
  if (length(tallennetut_kertoimet)<10) {
    ...
    retrieved_vars_previ_ecmos_narrow_v <- all_mos_and_derived_variables[match(retrieved_vars_previ_ecmos_narrow_v[["variable_name"]],all_mos_and_derived_variables[["variable_name"]]),]
    removed_vars <- retrieved_vars_previ_ecmos_narrow_v[which(is.na(row.match(retrieved_vars_previ_ecmos_narrow_v[,c("param_id","level_value")],unique(retrieved_data[,c("param_id","level_value")])))),]
    removed_vars <- removed_vars[sapply(removed_vars["param_id"],function (x) nchar(x)<7),]
    variable_list <- variable_list[which(is.na(row.match(variable_list,cbind.data.frame(removed_vars[,1],"previ_ecmos_narrow_v","MOS")))),]
    rm(removed_vars)
    ...
    rm(all_mos_and_derived_variables)
    rm(actually_retrieved_vars_previ_ecmos_narrow_v)
